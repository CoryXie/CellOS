/* tlsf.c - Two Levels Segregate Fit memory allocator (TLSF) */
#include <sys.h>
#include <arch.h>
#include <os.h>

/* Adapted for CELL64 based on TLSF 2.4.6 */
...
#if TLSF_USE_LOCKS
#define TLSF_MLOCK_T            spinlock_t
#define TLSF_CREATE_LOCK(l)     spinlock_init(l)
...
typedef unsigned char u8_t;     /* NOTE: Make sure that this type is 1 byte  */
...
static inline bhdr_t *process_area(void *area, size_t size)
    ...
    ib = (bhdr_t *) area;
    ib->size = (sizeof(area_info_t) < MIN_BLOCK_SIZE) ?
                MIN_BLOCK_SIZE :
                ROUNDUP_SIZE(sizeof(area_info_t)) | USED_BLOCK | PREV_USED;
    b = (bhdr_t *) GET_NEXT_BLOCK(ib->ptr.buffer, ib->size & BLOCK_SIZE);
    b->size = ROUNDDOWN_SIZE(size -
                             3 * BHDR_OVERHEAD -
...
size_t init_memory_pool(size_t mem_pool_size, void *mem_pool)
    ...
    PRINT_MSG("mem_pool %p, mem_pool_size %p done\n", mem_pool, mem_pool_size);
...
size_t add_new_area(void *area, size_t area_size, void *mem_pool)
    ...
    while (ptr)
        ...
        if ((unsigned long) ib1 == (unsigned long) lb0 + BHDR_OVERHEAD)
            ...
            b0->size = ROUNDDOWN_SIZE((b0->size & BLOCK_SIZE) +
                                  (ib1->size & BLOCK_SIZE) +
        ...
        if ((unsigned long) lb1->ptr.buffer == (unsigned long) ib0)
            ...
            lb1->size = ROUNDDOWN_SIZE((b0->size & BLOCK_SIZE) +
                     (ib0->size & BLOCK_SIZE) +
...
void *kmalloc(size_t size)
    ...
    if (!mp)
        ...
        area_size = (area_size > DEFAULT_AREA_SIZE) ?
...
void *malloc_ex(size_t size, void *mem_pool)
    ...
    /*
     * Searching a free block, recall that this function changes the
     * values of fl and sl, so they are not longer valid when the function
     * fails
     */
    ...
    if (!b)
        ...
        area_size = size + BHDR_OVERHEAD * 8;   /* size plus enough room
                                                 * for the requered headers.*/
        area_size = (area_size > DEFAULT_AREA_SIZE) ?
                    area_size : DEFAULT_AREA_SIZE;
...
void *realloc_ex(void *ptr, size_t new_size, void *mem_pool)
    ...
    new_size = (new_size < MIN_BLOCK_SIZE) ?
                MIN_BLOCK_SIZE : ROUNDUP_SIZE(new_size);
...
void print_block(bhdr_t * b)
    ...
    if ((b->size & BLOCK_STATE) == FREE_BLOCK)
        PRINT_MSG("free [%p, %p], ",
                  b->ptr.free_ptr.prev, b->ptr.free_ptr.next);
    else
        PRINT_MSG("used, ");
